 ...
 *  seems like free() doesn't purge the element from memory, it only allows it to be overwritten;
 *  so the pointers keeping their node values when freed is normal
 *
 *   Problem:  When lists are emptied, returning the pointer leaves us with an unchanged list, as if our free() did not do anything
 *   Solution: Return NULL instead of the pointer that was freed
 */
